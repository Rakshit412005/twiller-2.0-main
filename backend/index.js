...
app.post("/post", async (req, res) => {
  try {
    ...
    if (!author) return res.status(400).json({ error: "Author missing" });
    ...
    if (
      ...
      await User.updateOne(
        { _id: user._id },
        { plan: "free", tweetsUsed: 0, planExpiresAt: null }
      );
    }
...
app.post("/like/:tweetid", async (req, res) => {
  try {
    ...
    if (!tweet.likedBy.some(id => id.equals(userObjectId))) {
      tweet.likes += 1;
...
app.post("/retweet/:tweetid", async (req, res) => {
  try {
    ...
    const userObjectId = new mongoose.Types.ObjectId(userId);
    if (!tweet.retweetedBy.some(id => id.equals(userObjectId))) {
      tweet.retweets += 1;
